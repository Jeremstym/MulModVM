...
def load_datasets(hparams):
  if hparams.datatype == 'multimodal':
    ...
    train_dataset = ContrastiveImagingAndTabularDataset(
      ...
      hparams.data_val_eval_imaging, hparams.corruption_rate, hparams.field_lengths_tabular, hparams.one_hot,
    ...
    val_dataset = ContrastiveImagingAndTabularDataset(
      hparams.data_val_imaging_tensors, hparams.delete_segmentation, transform, hparams.augmentation_rate, 
...
def pretrain(hparams, wandb_logger):
  ...
  #   train_loader = DataLoader(train_dataset, shuffle=True, num_workers=num_workers, batch_size=hparams.batch_size, pin_memory=True)
  #   start = time()  
